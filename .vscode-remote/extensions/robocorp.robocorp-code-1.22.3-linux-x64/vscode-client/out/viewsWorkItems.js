...
async function getWorkItemInfo() {
    const resourcesTreeDataProvider = (viewsCommon_1.treeViewIdToTreeDataProvider.get(robocorpViews_1.TREE_VIEW_ROBOCORP_PACKAGE_RESOURCES_TREE));
    ...
    const currTreeDir = await (0, viewsRobotSelectionTreeBase_1.getCurrRobotDir)();
    if (!currTreeDir) {
        return;
    }
    const workItemsResult = await vscode.commands.executeCommand(robocorpCommands_1.ROBOCORP_LIST_WORK_ITEMS_INTERNAL, { robot: (0, path_1.resolve)(currTreeDir.filePath), "increment_output": false });
    if (!workItemsResult.success) {
        return;
    }
    return workItemsResult.result;
}
async function createNewWorkItem(workItemInfo, workItemName) {
    if (workItemName) {
    ...
    try {
        ...
        try {
            ...
            let ret = await vscode.window.showInformationMessage("File " + targetFile + " already exists.", { "modal": true }, OVERRIDE);
            if (ret != OVERRIDE) {
                return;
            }
        }
...
async function convertOutputWorkItemToInput(item) {
    if (item && item.kind == "outputWorkItem" && item.workItem) {
        ...
        let workItemName = await vscode.window.showInputBox({
        ...
        try {
            ...
            let OVERRIDE = "Override";
            let ret = await vscode.window.showInformationMessage("File " + target + " already exists.", { "modal": true }, OVERRIDE);
            if (ret != OVERRIDE) {
                return;
            }
        }
        ...
        try {
            ...
            await vscode.workspace.fs.copy(vscode.Uri.file((0, path_1.dirname)(workItem.json_path)), // src
...
async function newWorkItemInWorkItemsTree() {
...
async function deleteWorkItemInWorkItemsTree(item) {
    ...
    if (stat) {
        try {
            ...
            let msg = await vscode.window.showErrorMessage("Unable to move to trash: " + workItemPath + ". How to proceed?", { "modal": true }, DELETE_PERMANENTLY);
...
class WorkItemsTreeDataProvider extends viewsRobotSelectionTreeBase_1.RobotSelectionTreeDataProviderBase {
    ...
    async handleRoot() {
        ...
        const workItemsResult = await vscode.commands.executeCommand(robocorpCommands_1.ROBOCORP_LIST_WORK_ITEMS_INTERNAL, { "robot": robot, "increment_output": false });
        if (!workItemsResult.success) {
            this.workItemsInfo = undefined;
            return [
                {
     ...
     */
    async collectRpaFrameworkRequirementsErrorMessage(robot) {
        let interpreter = undefined;
        let interpreterResult = await (0, activities_1.resolveInterpreter)(robot);
        let msg = "";
        ...
        try {
            libraryVersionInfoActionResult = await vscode.commands.executeCommand(robocorpCommands_1.ROBOCORP_VERIFY_LIBRARY_VERSION_INTERNAL, {
